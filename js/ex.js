//exercises mapped as cardio or weighted
const exMap = {};
...
fetch("../php/exercise_api.php?action=fetch")
  ...
  .then(data => {
    data.forEach(ex => {
      exMap[ex.name] = ex.type;//exercises type
    });
    makeList();//checklist of exercises
  });
...
function togEx(ex, enabled) {
  ...
  //weighted: table for weight/reps/sets
  if (type === "Cardio") {
    box.innerHTML += `
  ...
  logArea.appendChild(box);//append everything to logArea 
}
...
add.addEventListener("click", () => {
  ...
  //listeners for exercise type buttons
...
remove.addEventListener("click", () => {
  ...
  marked.forEach(name => {
    //Check if exercise has logs before deleting
fetch(`../php/exercise_api.php?action=check_logs&name=${encodeURIComponent(name)}`) 
      ...
      .then(data => {
        ...
        if (!hasLogs || confirm(`This will delete all workout data for "${name}". Are you sure?`)) {
          ...
          .then(result => {
            if (result.success) {
              delete exMap[name];//remove from map
...
submit.addEventListener("click", () => {
  const summary = [];//summary of exercise
  const payload = [];//sent to backend
    ...
    fetch("../php/exercise_api.php?action=with_logs")
      ...
      .then(data => {
        ...
        if (payload.length > 0) {
          ...
          exSel.value = lastEx;
          updateMetricOptions(exTypes[lastEx]);//adjust axis
          loadProgress(lastEx);//reload graph
        }
      });
